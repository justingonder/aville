...
- **2026-04-28** — Flyer-ingestion pipeline shipped on branch `flyer-ingestion-design`, PR #3 opened against `main`. Spec finalized (Sections B + C drafted, decisions 5–10 locked, three self-review fixes inline). Plan written (13 tasks with verbatim code). All 13 tasks executed via subagent-driven-development; reviews and smoke tests caught 8 drift fixes that were committed as fix-up commits — most instructive was the threshold-calibration miss (plan specified `BUSINESS_CONFIDENT_MATCH = 0.85` based on hand-estimated SequenceMatcher math; actual realistic-case ratio was 0.833, recalibrated to 0.80 against empirical scores; saved as a feedback memory). End-to-end smoke against the Guesthouse flyer surfaced two deferred follow-ups (search returns listing pages; `extract_events` max_tokens=4096 truncation) — both documented in the "Flyer-ingestion pipeline" entry above. The "Flyer-ingestion pipeline" entry itself was updated from "in design — DRAFT spec exists" to "implemented 2026-04-28 — pending end-to-end validation against a real walk batch". 17 unit-test assertions all green; live Claude smoke against the Guesthouse photo correctly identified the event ("Wander Home Holiday Market — Mother's Day Edition", `seed_confidence: high`).