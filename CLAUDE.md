...
- **`LocalBusiness` schema + per-business landing pages + breadcrumbs** _(high priority — cross-cutting SEO + AI/LLM discovery)_ — currently each venue only exists as an `organizer` mention inside Events; there's no canonical entity page on aville.net. Add `/business/{slug}/` static pages with `LocalBusiness` JSON-LD (address, geo, telephone, priceRange, openingHours from existing `hours:` block, sameAs to socials). Biggest single SEO win for queries like "bars in Andersonville" or "[venue name] events" and helps knowledge-panel matching. Also the biggest remaining AI/LLM-discovery win: agents asking "what bars are in Andersonville?" or "what does Hopleaf offer?" should land on a canonical per-venue entity page. **Bundle with `BreadcrumbList` JSON-LD on event detail pages** (Home → Business → Event) — breadcrumbs only get rendered in Google SERPs when there are 2+ levels of depth, and the middle "Business" level needs a real on-site URL to point at, which is exactly what the new `/business/{slug}/` page gives us. Each business page should also get a markdown sibling (`/business/{slug}/index.md`) and be listed in `/llms.txt`. Templates + routes work, not a one-line change. Revisit when Claude Design credits reset (week of 2026-04-29). Priority bumped from medium to high 2026-04-23 after it came up independently in both SEO (2026-04-22) and AI discovery (2026-04-23) sessions. Noted 2026-04-22.
...
- **AI / LLM discovery Tier 2 — deferred items** _(low-medium priority)_ — follow-ups that would deepen agent-readiness on top of the Tier 1 bundle. In rough order of value:
  ...
  - **Per-business markdown pages** — `/business/{slug}/index.md` alongside the LocalBusiness HTML pages when those land (see higher-priority entry above). These would list every current event at the venue plus the venue's hours, address, website, etc. Also add each business URL to `/llms.txt`.
