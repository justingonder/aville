...
- **Malformed model output no longer aborts the whole run (2026-07-06 fix).** Claude occasionally emits an event with an off-vocabulary/missing `kind` (run-to-run variance). Such an event hits the `events.kind CHECK (kind IN ('recurring','dated'))` constraint in `upsert_event`; the resulting `sqlite3.IntegrityError` used to escape the *unguarded* upsert loop and kill the entire daily run — every business after the offender never processed (Atmosphere `/upcoming-events` did this on 2026-07-06). Two layers now contain it: `event_has_valid_kind(ev)` skips invalid-`kind` events before upsert (Fix A), and `try_upsert_event()` wraps the upsert in `try/except sqlite3.Error` so *any* per-event DB/constraint error is logged (`SKIP (db error): …`) and skipped rather than fatal (Fix B, defense in depth). **Behavior change:** a run with one bad event now succeeds and deploys minus that event, instead of failing loudly — consistent with how per-page fetch/extract failures already degrade. Both in `src/db.py`; tests in `scripts/test_pipeline_resilience.py`. (The "fail loud" RuntimeError still trips only when *every* extraction attempt fails — that guard is unchanged.)
