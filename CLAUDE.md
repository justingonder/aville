...
- Local admin UI: `scripts/admin.py` (Flask, runs at `localhost:5050`). Form-driven editing of `businesses.yaml` / `businesses_pending.yaml` / `tags.yaml` / `data/app.db` events with field validation, diff preview, and auto-commit per save (`git commit --only`, no auto-push). Localhost-bound — refuses non-loopback. Round-trip preserves YAML quote/block styles and ISO datetime tz suffixes; full implementation notes in [`docs/shipped.md`](docs/shipped.md). Adds `flask` + `ruamel.yaml` to `requirements.txt` (dev-only — admin doesn't deploy).
